...
void go_TSherlock(corpus* corp, int K, int K2, vector<int> dims, double lambda, double lambda2, double biasReg, int nEpoch, int iterations, const char* corp_name)
{
        TSherlock md(corp, K, K2, dims, lambda, lambda2, biasReg, nEpoch);
        md.init();
        md.train(iterations, 0.005);
        md.saveModel((string(corp_name) + "__" + md.toString() + ".txt").c_str());
        md.cleanUp();
...
int main(int argc, char** argv)
{
	...
	if (argc != 12) {
		...
		printf("12. Dim. of each layer -- only for Sherlock (try \"2|3|3|2\" for full clothing)\n");
		exit(1);
	}

	
	const char* metaPath = "productMeta_simple.txt.gz";
	...
	string ctpath(argv[11]);
	vector<string> CTPath;
	CTPath.push_back("Clothing\\ Shoes\\ \\&\\ Jewelry");
	if (ctpath != CTPath.back()){
		CTPath.push_back(ctpath);
	}
	...
	if (argv[12] != NULL){
		string dimstr(argv[12]);
		int start = 0;
		while (true) {
			size_t found = dimstr.find('|', start);
			if (found == string::npos) {
				dims.push_back(atoi(dimstr.substr(start).c_str()));
				break;
			} else {
				dims.push_back(atoi(dimstr.substr(start, found - start).c_str()));
				start = found + 1;
			}
		}
		layer = dims.size();
	...
	corpus corp;
        corp.loadData(imgFeatPath, metaPath, CTPath, layer - 1, reviewPath, 10, 0);
...
//	go_TVBPRplus(&corp, K, K2, lambda, lambda2, biasReg, nEpoch, iter, corp_name);
	go_TSherlock(&corp, K, K2, dims, lambda, lambda2, biasReg, nEpoch, iter, corp_name);
